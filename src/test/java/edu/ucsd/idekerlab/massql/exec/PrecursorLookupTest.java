...
class PrecursorLookupTest {
    ...
    // ---------------------------------------------------------------- rule 1: closest, not most intense
    ...
    void theCLOSESTpeakWinsEvenWhenAnotherInTheWindowIsFiveTimesMoreIntense() {
        ...
        // Wrong  : ms1_i = 5000.0 / ms1_precmz = 500.0078125   (the intuitive "most intense" reading)
        var r = PrecursorLookup.lookup(microMs1(), 2, 500.0, 20.0);
        assertEquals(1000.0, r.ms1I(),
    ...
    void bothCandidatesReallyAreInTheWindowSoTheTestAboveIsNotVacuous() {
        ...
        assertEquals(2, microMs1().mzWindow(0, 500.0 - tol, 500.0 + tol).size(),
                "the discriminating test needs BOTH peaks in the window");
    }

    // ---------------------------------------------------------------- rule 2: base peak survives a miss
    ...
    void ms1BasePeakIIsTheWholeScanMaxNotTheWindowMax() {
        // The complement of the above: even on a SUCCESSFUL match, the base peak comes from outside the
    ...
    void equidistantCandidatesResolveToTheLOWERMz() {
        ...
        //
        // ⚠ 40 ppm, NOT 20, and that matters. At 20 ppm tol is exactly 0.01, so both peaks land exactly
        // ON the window bounds and this test would also be exercising rule 4 -- it failed alongside the
        // C37 test during a sabotage run, which is how the overlap surfaced. At 40 ppm tol is 0.02 and
    ...
    void aPeakExactlyONTheBoundISAcandidate() {
        ...
        //
        // Why 7.8125: 500.0 * 7.8125 / 1e6 = 0.00390625, and 500.0 - 0.00390625 = 499.99609375 EXACTLY
        // -- the m/z of the first peak. So that peak sits precisely on the window's lower bound.
        double tol = 500.0 * 7.8125 / 1e6;
        assertEquals(499.99609375, 500.0 - tol,
    ...
    void theExclusiveVariantWouldHaveRejectedThatPeakWhichIsWhyTheChoiceMatters() {
        // Demonstrates the divergence directly on the store, so the C37 distinction is visible here and
        // not only in Tech_Step5's MzWindowTest.
        double tol = 500.0 * 7.8125 / 1e6;
        SpectrumTable t = microMs1();
        assertEquals(1, t.mzWindow(0, 500.0 - tol, 500.0 + tol).size(),
                "inclusive: the on-bound peak is in");
        assertEquals(0, t.mzWindowExclusive(0, 500.0 - tol, 500.0 + tol).size(),
    ...
    void theZeroSentinelOnMs1scanSuppressesTheLookupEvenIfATableIsPresent() {
        // This is why Tech_Step10 §4 converts sentinels AFTER the lookup: ms1scan == 0 is the signal.
        var r = PrecursorLookup.lookup(microMs1(), 0, 500.0, 20.0);
        assertNull(r.ms1BasePeakI(), "ms1scan == 0 means 'no linked scan', so nothing is looked up");
    }

    @Test
    void precmzZeroMeansNoLookupButTheBasePeakStillPopulates() {
        // precmz == 0 is "not recorded" -- it must NOT be matched against as if it were an m/z of zero.
    ...
    void aRetainedScanThatIsNotTheLinkedScanIsAnErrorNotASilentWrongAnswer() {
        // Tech_Step10 §3's C22 note: the document-order rule guarantees the retained MS1 IS ms1scan.
        ...
        assertTrue(e.getMessage().contains("document-order"),
