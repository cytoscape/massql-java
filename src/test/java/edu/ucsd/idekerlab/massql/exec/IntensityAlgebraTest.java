...
class IntensityAlgebraTest {
    ...
    private static Set<Integer> scans(String queryText, String fixture) {
        ...
        try (SpectraStream s = SpectraFile.open(resource(fixture))) {
            QueryExecutor.execute(Massql.parse(queryText), s, null,
                    (v, scan, ms1) -> out.add(v.scanId()));
        }
        return out;
    }
    ...
    void thePreconditionForDisjointnessActuallyHolds() {
        // Assert the precondition rather than assuming it -- otherwise the disjointness test below could
    ...
    void greaterThanAndLessThanAreDisjointUnderTheNarrowWindow() {
        ...
        assertFalse(gt.isEmpty() && lt.isEmpty(), "at least one side should be non-empty, or this is vacuous");
    }
    ...
    void equalsIsASupersetOfGreaterThanBecauseEqualsMeansGreaterOrEqual() {
        ...
        for (double threshold : new double[]{100, 1000, 1500, 4096}) {
            ...
            assertTrue(eq.containsAll(gt),
                    "at threshold " + threshold + ": INTENSITYVALUE= (>=) must contain every scan matched "
                            + "by INTENSITYVALUE>. gt=" + gt + " eq=" + eq);
        }
    }

    @Test
    void raisingAGreaterThanThresholdNeverAddsScans() {
        // Monotonicity, general. A wrong comparator direction shows up here even when the absolute counts
        // look plausible.
        ...
        for (double threshold : new double[]{0, 100, 1000, 1400, 100_000}) {
            ...
            if (prev != null) {
                assertTrue(prev.containsAll(cur),
                        "raising the threshold to " + threshold + " ADDED scans: was " + prev + ", now " + cur);
            }
            prev = cur;
        }
    ...
    void loweringALessThanCapNeverAddsScans() {
        ...
        for (double cap : new double[]{100_000, 1600, 1000, 100, 0}) {
            ...
            if (prev != null) {
                assertTrue(prev.containsAll(cur),
                        "lowering the cap to " + cap + " ADDED scans: was " + prev + ", now " + cur);
            }
            prev = cur;
        }
        assertEquals(Set.of(), prev, "a cap of 0 must match nothing -- the implicit floor is > 0 anyway");
    }

    @Test
    void noPercentThresholdCanBeMadeImpossibleByRaisingIt() {
        // An instructive consequence of the 0.99 cap, and one I got wrong when first writing this test:
        // ANY ">" threshold on a percent column is clamped to 0.99, so raising it cannot make the query
        // unsatisfiable. INTENSITYPERCENT>100000 still matches, because the threshold becomes 0.99 and
        // iNorm's maximum is exactly 1.0.
        //
        // That is the cap's whole purpose (the source's comment: "if people set it to 100, then they won't
        // get anything"), and it means monotonicity FLATTENS above 99 rather than continuing.
        assertEquals(Set.of(1, 3), scans(narrow(":INTENSITYPERCENT>100000"), FIXTURE),
                "the cap clamps the threshold to 0.99, so this is satisfiable -- raising a percent "
                        + "threshold is not a way to match nothing");
        assertEquals(scans(narrow(":INTENSITYPERCENT>100"), FIXTURE),
    ...
    void anAbsoluteThresholdCanBeMadeImpossible() {
        // INTENSITYVALUE has scale 1.0 and is never capped, so raising it DOES eventually match nothing --
    ...
    void aWindowThatContainsNoPeakIsEmptyAndDiagnosed() {
        // A tight tolerance around an EXACT peak still matches -- the peak sits at the window's CENTRE, not
        // its bound. To match nothing the TARGET must miss every peak, which is what this does: 200.6 is
        ...
        try (SpectraStream s = SpectraFile.open(resource(FIXTURE))) {
            ExecutionSummary sum = QueryExecutor.execute(Massql.parse(q), s, null, (v, scan, ms1) -> { });
            assertEquals(0, sum.qualifyingScans(), "no peak lies within 2e-7 of 200.6");
            assertFalse(sum.diagnostics().isEmpty(), "and it says so rather than returning silently");
        }
    }

    @Test
    void aTightToleranceAroundAnExactPeakStillMatches() {
        // The complement, so the test above cannot pass for the wrong reason: strict bounds exclude the
        ...
        try (SpectraStream s = SpectraFile.open(resource(FIXTURE))) {
            ExecutionSummary sum = QueryExecutor.execute(Massql.parse(q), s, null, (v, scan, ms1) -> { });
            assertEquals(2, sum.qualifyingScans(),
