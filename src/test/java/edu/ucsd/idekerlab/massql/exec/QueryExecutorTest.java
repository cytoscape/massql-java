...
class QueryExecutorTest {
    ...
    private static Path resource(String relative) {
        ...
        if (url == null) throw new AssertionError("fixture missing from src/test/resources: " + relative);
        try {
    ...
    private static Run run(String queryText, String fixture) {
        ...
        try (SpectraStream s = SpectraFile.open(resource(fixture))) {
            ExecutionSummary sum = QueryExecutor.execute(q, s, null,
                    (view, scan, ms1) -> scans.add(view.scanId()));
    ...
    void microEdgeQueryMatchesNothingBecauseBoundsAreStrict() {
        // ⛔ Correction C37(a). micro.mzML scan 3 has a peak at EXACTLY 201.0; this query's window is
        // [201.0, 202.0], so the peak sits precisely on the lower bound.
        //
        // MassQL returns 0 rows -- verified by execution, and micro_mzml_edge_results.json is that empty
        // golden. With an INCLUSIVE window this test returns [3] and the golden disagrees. It is the only
        // assertion in the suite that can catch the bound error, because no other query isolates that peak.
        Run r = run(query("test_micro_edge.massql"), "fixtures/micro/micro.mzML");
        assertEquals(List.of(), r.scans(),
        ...
        assertFalse(r.summary().diagnostics().isEmpty(),
                "an empty result must carry a diagnostic explaining itself (§5), not be silent");
    }

    @Test
    void microQueryMatchesTheGoldenScans() {
        // output/micro_mzml_results.json is [1, 3]. The query is MS2PROD=200.5:TOLERANCEMZ=0.5, so the
        // window is [200.0, 201.0] and 200.5 is strictly interior -- both scans that carry it qualify.
    ...
    void resultsArriveInScanIdAscendingOrder() {
        // Step 10 and Step 12 both require it, and the streaming design gets it free from document order --
    ...
    void aScanLevelOnlyQueryDoesNotReturnZeroPeakScans() {
        ...
        //
        // A peak-based condition fails an empty scan by itself. A SCAN-LEVEL condition never looks at peaks,
    ...
    void polarityOnAnMgfMatchesEveryLoadedScan() {
        // Correction C34(b), asserted deliberately rather than discovered: MGF polarity is a hardcoded 1
        // (C33), so POLARITY=Positive matches everything and POLARITY=Negative nothing. This is CORRECT
        // behaviour, not a broken filter -- it is in the Step 13 known-deviations list for that reason.
        assertEquals(21_942, run("QUERY scaninfo(MS2DATA) WHERE POLARITY=Positive",
                "data/PlusRise.mgf").summary().qualifyingScans());
        assertEquals(0, run("QUERY scaninfo(MS2DATA) WHERE POLARITY=Negative",
                "data/PlusRise.mgf").summary().qualifyingScans());
    }
    ...
    void twoConditionsMayBeSatisfiedByDifferentPeaksInTheSameScan() {
        // THE direct test of §1, and the most likely structural error in the step. micro.mzML scan 3 has
        // peaks at 100.0 and 300.0. No single peak can be both, so a row-level AND returns nothing while
        // scan-level intersection returns scan 3.
        Run r = run("QUERY scaninfo(MS2DATA) WHERE MS2PROD=100.0:TOLERANCEMZ=0.1 "
                + "AND MS2PROD=300.0:TOLERANCEMZ=0.1", "fixtures/micro/micro.mzML");
        assertTrue(r.scans().contains(3),
    ...
    void conditionOrderDoesNotChangeTheAnswer() {
        // Correction C37(g), proven from the source and pinned here. micro_ms1var.mzML is the ONLY fixture
        // that can discriminate: MS1 scan 1 has 400.0, MS1 scan 3 does not, and both MS2 scans carry 200.0.
        // small.mzML cannot -- its MS1 scans are profile-mode on an identical m/z grid.
        String a = "QUERY scaninfo(MS2DATA) WHERE MS1MZ=400.0:TOLERANCEMZ=0.1 AND MS2PROD=200.0:TOLERANCEMZ=0.1";
        String b = "QUERY scaninfo(MS2DATA) WHERE MS2PROD=200.0:TOLERANCEMZ=0.1 AND MS1MZ=400.0:TOLERANCEMZ=0.1";
    ...
    void ms1mzOverAnMgfCannotQualifyAndSaysWhy() {
        // MGF has no survey scans at all, so MS1MZ is unsatisfiable there. A silent empty result would be a
        // poor answer (§5); the diagnostic names the cause.
        Run r = run("QUERY scaninfo(MS2DATA) WHERE MS1MZ=100.0:TOLERANCEMZ=0.5", "fixtures/micro/micro.mgf");
        assertEquals(0, r.summary().qualifyingScans());
        assertTrue(r.summary().diagnostics().stream().anyMatch(d -> d.contains("MS1MZ")),
    ...
    void rtBoundsAreStrictWhileScanBoundsAreInclusive() {
        ...
        assertEquals(List.of(3, 5),
                run("QUERY scaninfo(MS2DATA) WHERE SCANMIN=3", "fixtures/micro/micro.mzML").scans());
        // SCANMAX=3 is INCLUSIVE -> scans 1 and 3.
        assertEquals(List.of(1, 3),
                run("QUERY scaninfo(MS2DATA) WHERE SCANMAX=3", "fixtures/micro/micro.mzML").scans());
        ...
        assertEquals(List.of(1),
                run("QUERY scaninfo(MS2DATA) WHERE RTMAX=1.0", "fixtures/micro/micro.mzML").scans());
    }

    // ------------------------------------------------------- the two conditions that had NO filter test
    //
    // CHARGE and MS2PREC are implemented in ConditionFilters (:71-72) and were exercised only by PARSE
    // tests -- AstShapeTest, KeywordCaseMatrixTest, ParseRejectionTest -- plus the two .massql fixtures
    // that Step 12's differential will eventually run. So Tech_Step9's exit criterion "every 9a and 9b
    // condition has a positive and a negative test" was checked while 2 of the 10 conditions had
    // neither, and nothing could tell: the whole filter could have been inverted and the suite stayed
    // green. Same shape as C36 and C37 -- a rule with nothing able to falsify it.

    @Test
    void chargeMatchesTheDeclaredChargeAndNothingElse() {
        // micro.mzML declares `charge state` on scan 5 ONLY (value 2); scans 1 and 3 omit it, so the
    ...
    void ms2precFiltersOnTheScansOwnPrecursorWithStrictBounds() {
        ...
        assertEquals(List.of(3, 5),
                run("QUERY scaninfo(MS2DATA) WHERE MS2PREC=500.0:TOLERANCEMZ=0.1",
                        "fixtures/micro/micro.mzML").scans());
        assertEquals(List.of(1),
                run("QUERY scaninfo(MS2DATA) WHERE MS2PREC=250.25:TOLERANCEMZ=0.1",
                        "fixtures/micro/micro.mzML").scans());

        // Negative: no scan carries this precursor.
        assertEquals(List.of(),
                run("QUERY scaninfo(MS2DATA) WHERE MS2PREC=999.0:TOLERANCEMZ=0.1",
                        "fixtures/micro/micro.mzML").scans());

        // MS2PREC reads the scan's OWN precmz -- it must not be confused with MS2PROD, which searches
        ...
        assertEquals(List.of(),
                run("QUERY scaninfo(MS2DATA) WHERE MS2PROD=500.0:TOLERANCEMZ=0.1",
                        "fixtures/micro/micro.mzML").scans(),
    ...
    void ms2precBoundsAreStrictNotInclusive() {
        ...
        // means it can drift independently of the store. A tolerance placing the window edge exactly
        ...
        assertEquals(List.of(),
                run("QUERY scaninfo(MS2DATA) WHERE MS2PREC=500.5:TOLERANCEMZ=0.5",
                        "fixtures/micro/micro.mzML").scans(),
        ...
        assertEquals(List.of(3, 5),
                run("QUERY scaninfo(MS2DATA) WHERE MS2PREC=500.4:TOLERANCEMZ=0.5",
                        "fixtures/micro/micro.mzML").scans());
    }
    ...
    // the only condition whose match target is DERIVED (precmz - loss) rather than given, so a wrong
    ...
    void ms2nlIsComputedFromEachScansOwnPrecursor() {
        ...
        assertEquals(List.of(3),
                run("QUERY scaninfo(MS2DATA) WHERE MS2NL=200.0:TOLERANCEMZ=0.1",
                        "fixtures/micro/micro.mzML").scans());

        // THE test that the precursor is per-scan rather than global: 250.25 - 49.75 = 200.5, which is
        // a peak in scan 1. Scan 3 also has a 200.5 peak, but its precmz is 500.0, so the same loss
        // points at 450.25 where it has nothing. A shared or last-seen precmz returns the wrong set.
        assertEquals(List.of(1),
                run("QUERY scaninfo(MS2DATA) WHERE MS2NL=49.75:TOLERANCEMZ=0.1",
                        "fixtures/micro/micro.mzML").scans(),
        ...
        assertEquals(List.of(),
                run("QUERY scaninfo(MS2DATA) WHERE MS2NL=1.0:TOLERANCEMZ=0.1",
                        "fixtures/micro/micro.mzML").scans());
    }
    ...
    void ms2nlBoundsAreStrict() {
        ...
        assertEquals(List.of(),
                run("QUERY scaninfo(MS2DATA) WHERE MS2NL=199.5:TOLERANCEMZ=0.5",
                        "fixtures/micro/micro.mzML").scans(),
        ...
        assertEquals(List.of(3),
                run("QUERY scaninfo(MS2DATA) WHERE MS2NL=200.0:TOLERANCEMZ=0.5",
                        "fixtures/micro/micro.mzML").scans());
    }
    ...
    void anMs2ScanWithNoRecordedPrecursorCannotSatisfyMs2nl() {
        ...
        // reference excludes such a scan only incidentally -- (0 - mz) is negative while the window is
        ...
        //
        // Note MassQL cannot load this file at all (KeyError: 'precursorMz', C27c), so this pins OUR
        // contract rather than parity -- which is exactly why it needs stating in a test.
        assertEquals(List.of(),
                run("QUERY scaninfo(MS2DATA) WHERE MS2NL=18.0:TOLERANCEMZ=0.5",
                        "fixtures/micro/micro_noprecursor.mzXML").scans(),
                "a scan with no recorded precursor must be excluded, not matched against a precmz of 0");

        // And it is not vacuous -- the same fixture DOES qualify on a peak-level condition that needs
        // no precursor, so the emptiness above is the MS2NL rule and not an unreadable file.
        assertFalse(run("QUERY scaninfo(MS2DATA) WHERE MS2PROD=200.5:TOLERANCEMZ=0.5",
                        "fixtures/micro/micro_noprecursor.mzXML").scans().isEmpty(),
                "if this is empty too, the test above proves nothing about MS2NL");
    }

    @Test
    void anOrValueListIsSatisfiedByAnyValue() {
        // Union, matching pd.concat over the per-value frames. 100.0 is in scans 1 and 3; 123.45… only in 5.
        Run r = run("QUERY scaninfo(MS2DATA) WHERE MS2PROD=(100.0 OR 123.456789012345):TOLERANCEMZ=0.01",
