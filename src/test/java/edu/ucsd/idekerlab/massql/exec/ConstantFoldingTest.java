...
class ConstantFoldingTest {
    ...
    private static Expr lit(double v) { return new Expr.Literal(v); }
    private static Expr bin(double l, Op op, double r) { return new Expr.Binary(lit(l), op, lit(r)); }
    ...
    void unaryNegationFolds() {
        ...
        assertEquals(18.0, ConstantFolder.fold(new Expr.Unary(Op.ADD, lit(18.0))), "unary plus is identity");
    }
    ...
    void unaryNestsAndCombinesWithBinary() {
        ...
        // Double negation.
        assertEquals(18.0, ConstantFolder.fold(new Expr.Unary(Op.SUB, new Expr.Unary(Op.SUB, lit(18.0)))));
    }

    @Test
    void aUnaryMultiplyOrDivideCannotEvenBeCONSTRUCTED() {
        // Validation lives in Expr.Unary's own constructor -- a better place than the folder, since it makes
        // the invalid state unrepresentable rather than merely unevaluatable. The folder's own guard is
        // therefore defensive and unreachable, which is why this asserts IllegalArgumentException from the
    ...
    void ieeeSemanticsArePreservedNotTidiedUp() {
        // MassQL does double arithmetic; matching it bit-for-bit is the goal. Rounding 0.1+0.2 to a "nice"
        // 0.3 would put every downstream tolerance window off by an ULP.
        assertEquals(Double.doubleToLongBits(0.1 + 0.2),
                Double.doubleToLongBits(ConstantFolder.fold(bin(0.1, Op.ADD, 0.2))));
        assertNotEquals(Double.doubleToLongBits(0.3),
                Double.doubleToLongBits(ConstantFolder.fold(bin(0.1, Op.ADD, 0.2))),
    ...
    void divisionByZeroYieldsInfinityRatherThanThrowing() {
        // An infinite target simply matches nothing, which is the right outcome for a degenerate query.
    ...
    void precedenceComesFromTheAstNotFromReparsing() {
        // The folder walks whatever tree it is given; grouping is the parser's job. 1 + (2 * 3) = 7,
        // while (1 + 2) * 3 = 9 -- same tokens, different trees, and the folder must not "fix" either.
        Expr rightHeavy = new Expr.Binary(lit(1.0), Op.ADD, new Expr.Binary(lit(2.0), Op.MUL, lit(3.0)));
        Expr leftHeavy = new Expr.Binary(new Expr.Binary(lit(1.0), Op.ADD, lit(2.0)), Op.MUL, lit(3.0));
