...
class IntensitySemanticsTest {
    ...
    void theThreeScalesUseThreeDifferentColumns() {
        ...
        assertTrue(IntensityQualifiers.rowQualifies(t, 1,
                List.of(q(QualifierType.INTENSITYVALUE, Comparator.GT, 40))));
        assertTrue(IntensityQualifiers.rowQualifies(t, 1,
                List.of(q(QualifierType.INTENSITYPERCENT, Comparator.GT, 40))));
        assertFalse(IntensityQualifiers.rowQualifies(t, 1,
                List.of(q(QualifierType.INTENSITYTICPERCENT, Comparator.GT, 40))),
    ...
    void bothPercentQualifiersDivideByOneHundred() {
        ...
        assertTrue(IntensityQualifiers.rowQualifies(t, 1,
                List.of(q(QualifierType.INTENSITYPERCENT, Comparator.EQ, 50))), "0.5 >= 0.50");
        assertFalse(IntensityQualifiers.rowQualifies(t, 1,
                List.of(q(QualifierType.INTENSITYPERCENT, Comparator.GT, 50))), "0.5 > 0.50 is false");

        // Row 1: iTicNorm = 0.3. A qualifier of 30 means 0.30 -- proving the tic column is ALSO divided.
        assertTrue(IntensityQualifiers.rowQualifies(t, 1,
                List.of(q(QualifierType.INTENSITYTICPERCENT, Comparator.EQ, 30))), "0.3 >= 0.30");
        assertFalse(IntensityQualifiers.rowQualifies(t, 1,
                List.of(q(QualifierType.INTENSITYTICPERCENT, Comparator.GT, 30))), "0.3 > 0.30 is false");
        // If the tic column were NOT divided, a qualifier of 30 would mean 30.0 and nothing would match.
    }
    ...
    void intensityValueIsNotDivided() {
        SpectrumTable t = scan();
        assertTrue(IntensityQualifiers.rowQualifies(t, 1,
                List.of(q(QualifierType.INTENSITYVALUE, Comparator.EQ, 300))), "300 >= 300 absolute");
        assertFalse(IntensityQualifiers.rowQualifies(t, 1,
                List.of(q(QualifierType.INTENSITYVALUE, Comparator.EQ, 301))));
    }
    ...
    void equalsMeansGreaterOrEqual() {
        SpectrumTable t = scan();
        // THE rule most likely to be "fixed" by a well-meaning implementer. The source's own comment:
        // "equal -> minimum threshold (>= val), preserving historical semantics".
        assertTrue(IntensityQualifiers.rowQualifies(t, 1,
                List.of(q(QualifierType.INTENSITYVALUE, Comparator.EQ, 300))), "exactly equal matches");
        assertTrue(IntensityQualifiers.rowQualifies(t, 2,
                List.of(q(QualifierType.INTENSITYVALUE, Comparator.EQ, 300))), "600 >= 300 also matches");
        assertFalse(IntensityQualifiers.rowQualifies(t, 0,
                List.of(q(QualifierType.INTENSITYVALUE, Comparator.EQ, 300))), "100 >= 300 does not");
        // If "=" were true equality, row 2 (600) would NOT match -- that is the discriminating case.
    }

    @Test
    void lessThanIsStrict() {
        SpectrumTable t = scan();
        assertTrue(IntensityQualifiers.rowQualifies(t, 0,
                List.of(q(QualifierType.INTENSITYVALUE, Comparator.LT, 300))), "100 < 300");
        assertFalse(IntensityQualifiers.rowQualifies(t, 1,
                List.of(q(QualifierType.INTENSITYVALUE, Comparator.LT, 300))), "300 < 300 is false");
    }
    ...
    void theCapAppliesToBothPercentQualifiersButOnlyForGreaterThan() {
        ...
        //
        // Without the cap, INTENSITYPERCENT>100 becomes "> 1.0", which NOTHING can satisfy since iNorm's
        // maximum is exactly 1.0. The source clamps the threshold to 0.99 instead.
        assertTrue(IntensityQualifiers.rowQualifies(t, 2,
                List.of(q(QualifierType.INTENSITYPERCENT, Comparator.GT, 100))),
                "INTENSITYPERCENT>100 must become > 0.99, so the base peak matches");

        // C37(d): the guard is `if scale > 1.0`, so INTENSITYTICPERCENT is capped too -- Known traps used to
        // say "INTENSITYPERCENT only". iTicNorm = 0.6 > 0.99 is false, so use a row that exceeds the cap...
        // no single peak can, since iTicNorm sums to 1 across the scan. So assert the threshold clamping
        ...
        assertTrue(IntensityQualifiers.rowQualifies(single, 0,
                List.of(q(QualifierType.INTENSITYTICPERCENT, Comparator.GT, 100))),
                "INTENSITYTICPERCENT>100 must ALSO be capped to > 0.99 (C37d)");

        // The cap does NOT apply to ">=": 1.0 >= 1.0 already holds, so this passes either way -- the
        // discriminating case is that it must not LOOSEN a >= threshold above 0.99.
        assertFalse(IntensityQualifiers.rowQualifies(t, 1,
                List.of(q(QualifierType.INTENSITYPERCENT, Comparator.EQ, 100))),
    ...
    void theCapNeverAppliesToTheAbsoluteColumn() {
        ...
        assertFalse(IntensityQualifiers.rowQualifies(t, 1,
                List.of(q(QualifierType.INTENSITYVALUE, Comparator.GT, 500))), "300 > 500 is false");
        assertTrue(IntensityQualifiers.rowQualifies(t, 2,
                List.of(q(QualifierType.INTENSITYVALUE, Comparator.GT, 500))), "600 > 500");
        // Were the cap applied here, a threshold of 500 would become 0.99 and BOTH rows would match.
    }
    ...
    void anAbsentQualifierMeansImplicitGreaterThanZeroPerColumn() {
        // C37(e): the floor is applied to all THREE columns independently, not as one blanket check.
        SpectrumTableBuilder b = new SpectrumTableBuilder(2);
        b.startScan(1, 0.5, 1);
        b.addPeak(100.0, 0).addPeak(200.0, 500);       // a zero-intensity peak and a real one
        SpectrumTable t = b.build();

        assertFalse(IntensityQualifiers.rowQualifies(t, 0, List.of()),
        ...
        assertFalse(IntensityQualifiers.rowQualifies(t, 0, null), "null qualifiers behave as absent");
    }

    @Test
    void anUnqualifiedColumnKeepsItsFloorEvenWhenAnotherIsQualified() {
        // The per-column detail, made observable: qualifying INTENSITYVALUE does not remove the implicit
        ...
        // A generous absolute threshold cannot rescue the row: iNorm/iTicNorm still face > 0, and NaN
        // fails every comparison -- which is also how pandas behaves.
        assertFalse(IntensityQualifiers.rowQualifies(t, 0,
                List.of(q(QualifierType.INTENSITYVALUE, Comparator.LT, 1_000_000))),
