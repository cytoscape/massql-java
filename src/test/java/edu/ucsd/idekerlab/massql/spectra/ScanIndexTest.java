...
class ScanIndexTest {
    ...
    void rowRangesPartitionTheTableExactly() {
        ...
        assertEquals(0, x.rowStart(0)); assertEquals(2, x.rowEnd(0));
        assertEquals(2, x.rowStart(1)); assertEquals(3, x.rowEnd(1));
        assertEquals(3, x.rowStart(2)); assertEquals(5, x.rowEnd(2));
        assertEquals(t.rowCount(), x.rowEnd(x.scanCount() - 1), "the last range must reach the end");
    }
    ...
    void retentionTimeIsExactAtDoublePrecision() {
        ...
        assertNotEquals(golden, (double) (float) golden, "the value genuinely does not survive float");
    }
