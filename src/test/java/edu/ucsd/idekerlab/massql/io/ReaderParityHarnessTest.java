...
class ReaderParityHarnessTest {
    ...
    void hexParsingRoundTripsExactly() {
        // The dumps store floats as Java/C99 hex literals precisely so no decimal rounding intervenes.
        double[] awkward = {
                0.0, -0.0, 1.0, 0.023,
        ...
        for (double v : awkward) {
            ...
            assertEquals(Double.doubleToLongBits(v), Double.doubleToLongBits(ParityDump.parseHex(hex)),
    ...
    void hexParsingReadsThePythonEmittedForm() {
        // Python's float.hex() writes "0x1.436b8f9b13166p-8"; Java's Double.toHexString writes the same
        // shape. Assert against literals lifted from an actual dump, so a generator change is caught here
        // rather than as 900 mysterious failures downstream.
        assertEquals(Double.doubleToLongBits(0.004935),
        ...
        assertEquals(0.0, ParityDump.parseHex("0x0.0p+0"), "the all-zero form used by empty MGF sums");
    }
    ...
    void aDecimalRoundTripWouldHaveLostBitsHere() {
        ...
        // "%.6f"-style dump would keep, and it is not enough: the value comes back a different double.
        double v = 0.011218333333333334;
        double viaDecimal = Double.parseDouble(String.format("%.6f", v));
        assertNotEquals(Double.doubleToLongBits(v), Double.doubleToLongBits(viaDecimal),
    ...
    void aSingleBitPerturbationChangesTheDigest() {
        // THE assertion that makes this gate meaningful. A decoder reading 8 bytes where 4 were written
        ...
        assertNotEquals(clean, ParityDump.sha256Of(perturbed),
        ...
        assertEquals(1, Math.abs(Double.doubleToLongBits(perturbed[2])
                        - Double.doubleToLongBits(values[2])),
                "the perturbation should differ in exactly the last bit");
    }

    @Test
    void reorderingChangesTheDigest() {
        // The INVERSE of what the spec originally asked for (C32d): it wanted a multiset comparator where
        // reordering compares EQUAL. The dumps store digests, which are order-sensitive, and that is a
        // feature -- our array order must match MassQL's file order. PeakOrderPreconditionTest asserts the
        // precondition that makes this safe.
        double[] a = {100.0, 200.0, 300.0};
        double[] b = {100.0, 300.0, 200.0};
        assertNotEquals(ParityDump.sha256Of(a), ParityDump.sha256Of(b),
                "the digest must be order-sensitive; if it were not, a reordered decode would pass");
    }

    @Test
    void signedZeroAndNaNAreDistinguished() {
        // -0.0 == 0.0 under ==, so a harness comparing with == would treat them as identical. The digest
        // must not, because they are different bits and MassQL would have produced one specific one.
        assertNotEquals(ParityDump.sha256Of(new double[]{0.0}), ParityDump.sha256Of(new double[]{-0.0}),
                "0.0 and -0.0 are different bits and must digest differently");
        assertEquals(ParityDump.sha256Of(new double[]{Double.NaN}),
    ...
    void theDigestMatchesTheDumpForARealScan() {
        // End to end on real data: recompute a digest from OUR decode and compare to what Python wrote.
        // If this fails, either the packing convention or the decode is wrong, and every other test in
    ...
    void aMissingDumpFailsRatherThanSkipping() {
        // C26 again: absence must be loud. If this ever returns instead of throwing, the gate can be
    ...
    void theDumpKeyIsMslevelPlusScanNotScanAlone() {
        // C32a, asserted rather than trusted: micro.mgf's synthetic MS1 has scan id 3, which is ALSO a
        // real MS2 id. Under a scan-id-only key one of the two would overwrite the other and the harness
        ...
        // Correction C33: the fake MS1 row is NOT always an all-zero placeholder. MassQL's pyteomics MGF
        // loader ends with `ms1_df = pd.DataFrame([peak_dict])` where peak_dict LEAKS from the MS2 peak
        // loop, so the row is a byte-for-byte DUPLICATE of the last MS2 peak. The all-zero form
        // (scan=1, mz=0, i=0) is only the `except` branch, reached when the loop never ran -- which is
        // PlusRise's case via the manual-loader fallback.
        //
        // Both flavours are artifacts our reader correctly omits (MGF has no survey scans), which is why
        ...
        // micro.mgf takes the pyteomics path, so its fake row duplicates the LAST MS2 peak -- meaning a
        // REAL intensity, not zero. Asserting 0.0 here (as this test first did) fails on correct data.
        assertNotEquals(0.0, phantom.iSum(),
        ...
        ParityDump.Scan plusRiseFake = ParityDump.of("PlusRise.mgf").scans().get(new ParityDump.Key(1, 1));
        assertNotNull(plusRiseFake, "PlusRise's fake MS1 sits at scan 1");
        assertEquals(0.0, plusRiseFake.iSum(),
    ...
    void firstEightValuesParseAndAreUsableForDiagnosis() {
        ...
        assertTrue(first > 0.0 && first < 100_000.0, "leading m/z should be a plausible mass, got " + first);
        // Ascending, which is what makes them a usable ordering diagnostic.
        for (int i = 1; i < mzHex.size(); i++) {
            assertTrue(ParityDump.parseHex(mzHex.get(i)) >= ParityDump.parseHex(mzHex.get(i - 1)),
