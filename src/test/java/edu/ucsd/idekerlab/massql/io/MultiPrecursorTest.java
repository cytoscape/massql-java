...
class MultiPrecursorTest {
    ...
    void mzxmlTakesTheFirstPrecursorNotTheLast() {
        ...
        got.forEach((scan, pc) -> {
            assertNotEquals(DECOY_MZ, pc[0],
                    "scan " + scan + " reported the SECOND <precursorMz> (" + DECOY_MZ + "). MassQL "
                            + "hard-indexes precursorMz[0] -- first wins, not last (C31)");
            assertNotEquals(7.0, pc[1],
                    "scan " + scan + " took charge from the second <precursorMz>; MassQL reads "
    ...
    void mzmlTakesTheFirstSelectedIonOfTheFirstPrecursor() {
        ...
        got.forEach((scan, pc) -> {
            // The fixture carries BOTH a second <selectedIon> inside the first <precursor> AND a
            // whole second <precursor>, so this catches a reader that respects one level but not the other.
            assertNotEquals(DECOY_MZ, pc[0],
                    "scan " + scan + " reported the second <selectedIon> (" + DECOY_MZ + "); MassQL "
                            + "reads selectedIon[0] (C31)");
            assertNotEquals(DECOY_MZ_2, pc[0],
                    "scan " + scan + " reported the second <precursor> (" + DECOY_MZ_2 + "); MassQL "
    ...
    void theFixturesReallyDoCarryMultiplePrecursors() {
        ...
        assertTrue(countOf(mzxml, "<precursorMz") >= 6,
    ...
    void everythingElseMatchesTheSinglePrecursorBaseline() {
        // The decoys must perturb nothing but the precursor fields. Compare against the plain fixtures
        ...
        assertEquals(read.apply("micro.mzXML"), read.apply("micro_multiprec.mzXML"),
                "extra precursors changed something other than the precursor fields (mzXML)");
        assertEquals(read.apply("micro.mzML"), read.apply("micro_multiprec.mzML"),
    ...
    private static String read(String fixture) {
        try {
            return new String(java.nio.file.Files.readAllBytes(
                    Fixtures.require("fixtures/micro/" + fixture)),
