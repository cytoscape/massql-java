...
class MzxmlPolarityTest {
    ...
    class Parity {
        ...
        void theRealFixtureIsAllPositive() {
            // small.mzXML declares polarity="+" on all 48 scans, so every scan must report 1. If this
            // ever reports 0, the attribute is not being read at all -- and because absent also maps
    ...
    class OurContractNotParity {
        ...
        void absentPolarityIsZeroAndMassqlWouldHaveRaised() {
            // NO GOLDEN EXISTS for this. MassQL raises KeyError: 'polarity' on micro_nopolarity.mzXML
            // (verified). We choose 0 -- the same "not recorded" value the format's own absent case
            // gets in mzML -- because crashing on a legal optional attribute is worse behaviour than
            // the reference, not better.
            assertEquals(0, MzxmlReader.polarityOf(null),
        ...
        void everythingElseAboutThatFileIsUnchanged() {
            // The absent attribute must not perturb anything else -- otherwise "polarity 0" could be
            // masking a broken parse. Compare against the baseline fixture, ignoring polarity.
            record Row(int scan, int level, int ms1scan, double rt, double precmz, int charge, int peaks) { }
            ...
            assertEquals(read.apply("micro.mzXML"), read.apply("micro_nopolarity.mzXML"),
