...
class ParseRejectionTest {
    ...
    void bareFunctionNameWithoutParensIsASyntaxErrorThatExplainsItself() {
        MassqlParseException e = assertThrows(MassqlParseException.class,
                () -> Massql.parse("QUERY scaninfo WHERE MS2PROD=100"));
        assertTrue(e.getMessage().contains("scaninfo(MS2DATA)"),
    ...
    void theOtherFiveFunctionsRejectByName(String query, String construct) {
        MassqlParseException e = assertThrows(MassqlParseException.class, () -> Massql.parse(query));
        assertEquals(construct, e.construct());
        assertTrue(e.getMessage().contains("scaninfo"), "message should point at what IS supported");
    }
    ...
    void cardinalityRejectsUnderBothSpellings() {
        for (String kw : new String[]{"CARDINALITY", "MATCHCOUNT"}) {
            MassqlParseException e = assertThrows(MassqlParseException.class, () -> Massql.parse(
                    "QUERY scaninfo(MS2DATA) WHERE MS2PROD=(100 OR 200):" + kw + "=range(min=2,max=5)"));
    ...
    @ParameterizedTest
    @ValueSource(strings = {
        "QUERY scaninfo(MS2DATA) where MS2PROD=100 filter MS2PROD=200", // 'filter' has no lowercase form
        ...
        "QUERY scaninfo(MS2DATA) WHERE MS2PROD=100:toleranceppm=5",     // qualifier names likewise
    ...
    void garbageInputNeverCrashes() {
        ...
        String[] junk = {"", "   ", "\n", "(((", ":::", "=", "QUERY", "QUERY scaninfo(",
                         "QUERY scaninfo(MS2DATA) WHERE", "\t\t", "0", "()"};
        for (String j : junk) {
            assertThrows(MassqlParseException.class, () -> Massql.parse(j),
