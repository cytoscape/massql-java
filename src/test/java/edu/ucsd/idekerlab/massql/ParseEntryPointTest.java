...
class ParseEntryPointTest {
    ...
    void surroundingWhitespaceAndTrailingNewlinesAreIgnored() {
        ...
        for (String variant : new String[]{Q + "\n", "\n" + Q, "  " + Q + "  ", "\t" + Q + "\n\n", "\r\n" + Q + "\r\n"}) {
            assertEquals(canonical, Massql.parse(variant).canonical(),
                    "whitespace variant changed the parse: " + variant.replace("\n", "\\n").replace("\r", "\\r"));
        }
    }
    ...
    void internalWhitespaceIsFlexible() {
        ...
        assertEquals(Massql.parse(Q).canonical(),
                Massql.parse("QUERY  scaninfo( MS2DATA )  WHERE  MS2PROD = 226.18 : TOLERANCEPPM = 5").canonical());
    }

    @Test
    void blankInputIsRejectedClearly() {
        for (String blank : new String[]{"", "   ", "\n", "\t", "  \n  "}) {
            MassqlParseException e = assertThrows(MassqlParseException.class, () -> Massql.parse(blank));
    ...
    void positionIsReportedOneBasedWhenKnown() {
        ...
        assertTrue(e.position() >= 1, "position should be 1-based and positive, got " + e.position());
    }
}
