 ...
 *
 * <p><b>The contract is described in {@code docs/RESULT_SCHEMA.md}</b>, which carries the key order,
 * the per-field nullability, the population-by-format table and the float policy. ⛔ Do not restate the
 * contract in javadoc: {@code KEYS} below is the definition, and {@code ResultJsonTest} pins it.
 *
 ...
 * is ever omitted. This record therefore has <b>no</b> {@code ms1DataShape} flag: with one shape such a
 * flag selects nothing, and it would be a per-<i>query</i> property stored per-<i>row</i> — 664 identical
 * copies in a PlusRise result, and part of {@code equals}.
 *
...
public record ScanInfoResult(
     ...
     *
     * <p>⛔ This list is the definition of the key set and its order. {@code ResultJsonTest} asserts
     * that {@code ResultJson} emits exactly these keys, in exactly this order.
     */
