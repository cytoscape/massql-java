...
[`SPIKE.md`](harness/SPIKE.md) §11 asked eight questions. This file answers all eight and cites the
...
> Evidence: [`PARITY_REPORT.md`](harness/PARITY_REPORT.md) — verdict GREEN, 16/16 fixtures.
...
> [`DIFFERENTIAL_REPORT.md`](harness/DIFFERENTIAL_REPORT.md). Correction C11.
...
> Evidence: [`dependency-audit.txt`](../dependency-audit.txt), regenerated by `make audit` and gated in
> CI. Correction C2 for the fork question.
...
In-process per fixture, every one completes in under 350 ms. [`SPIKE.md`](harness/SPIKE.md) §7 flagged
...
> captured programmatically; [`DIFFERENTIAL_REPORT.md`](harness/DIFFERENTIAL_REPORT.md).
...
[`FEATURE_MATRIX.md`](FEATURE_MATRIX.md) for exactly what parses, executes and rejects.
